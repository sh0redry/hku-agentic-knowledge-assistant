...
export function apply(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Search Find@HKUL through a fixed public Primo route and read up to 20 visible bibliographic results. Browser navigation occurs, but the tool never signs in, opens licensed full text, saves favorites, requests an item, or performs a library write. Treat availability labels as observations that can change.',
        parameters: {
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Open the fixed Find@HKUL full-display route for one stable record ID and read visible bibliographic fields. This never opens licensed full text, signs in, saves, requests, or returns authentication links.',
        parameters: {
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Read visible online or physical availability labels for one Find@HKUL record. It suppresses proxy, SSO, and full-text URLs and performs no Library write or full-text navigation.',
        parameters: {
            record_id: { type: 'string', required: true, description: 'Stable record_id returned by hku_library_research_search.' },
        },
        output: envelopeOutput,

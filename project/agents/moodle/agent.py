...
class MoodleDashboardInspectCapability(BaseCapability):
    ...
    async def execute(
        ...
        try:
            ...
            if exc.code in {"COMMAND_NOT_ALLOWED", "PAGE_SCRIPT_UNAVAILABLE"}:
                raise CapabilityError(
                    ...
                    "Reload HKU AGENTS Browser Bridge 0.17.5 and refresh HKU Portal and Moodle.",
                ) from exc
        ...
        if not _supported_parser(diagnostics.get("parser_version"), (0, 4, 1)):
            raise CapabilityError(
                "EXTENSION_UPDATE_REQUIRED",
                "Reload HKU AGENTS Browser Bridge 0.17.5 before Moodle inspection.",
            )
...
class MoodleCourseListCapability(BaseCapability):
    ...
    async def execute(
        ...
        if not _supported_parser(diagnostics.get("parser_version"), (0, 4, 1)):
            raise CapabilityError(
                "EXTENSION_UPDATE_REQUIRED",
                "Reload HKU AGENTS Browser Bridge 0.17.5 before listing Moodle courses.",
            )
...
class MoodleUpcomingAssignmentsCapability(BaseCapability):
    ...
    async def execute(
        ...
        if not _supported_parser(diagnostics.get("parser_version"), (0, 4, 1)):
            raise CapabilityError(
                "EXTENSION_UPDATE_REQUIRED",
                "Reload HKU AGENTS Browser Bridge 0.17.5 before reading Moodle assignments.",
            )

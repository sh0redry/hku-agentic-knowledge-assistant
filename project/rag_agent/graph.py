...
def create_agent_graph(llm, tools_list, collection, rewrite_llm=None):
    ...
    graph_builder.add_node("summarize_history", partial(summarize_history, llm=rewrite_llm, fallback_llm=llm))
    graph_builder.add_node("rewrite_query", partial(rewrite_query, llm=rewrite_llm, fallback_llm=llm))
    graph_builder.add_node(request_clarification)
    graph_builder.add_node("direct_answer", partial(direct_answer, llm=llm, collection=collection))
